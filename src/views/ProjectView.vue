...
async function openTask(task) { selectedTask.value = task; const r = await supabase.from('comments').select('*, author:profiles(full_name)').eq('task_id', task.id).order('created_at'); comments.value = r.data || [] }
...
<template><AppLayout><div v-if="!project" class="empty">{{ error || 'Загрузка проекта...' }}</div><template v-else>
  ...
  <div v-if="selectedTask" class="modal-bg" @click.self="selectedTask = null"><section class="modal"><button class="close" @click="selectedTask = null">×</button><span :class="['priority', selectedTask.priority]">{{ priorityName(selectedTask.priority) }}</span><h2>{{ selectedTask.title }}</h2><p>{{ selectedTask.description || 'Без описания' }}</p><p><b>Исполнитель:</b> {{ selectedTask.assignee?.full_name || 'не назначен' }}</p><p><b>Срок:</b> {{ selectedTask.due_date ? new Date(selectedTask.due_date).toLocaleDateString('ru') : 'не указан' }}</p><button class="danger-outline small" @click="removeTask(selectedTask)">Удалить задачу</button><hr/><h3>Комментарии</h3><div class="comments"><div v-for="item in comments" :key="item.id" class="comment"><b>{{ item.author?.full_name || 'Пользователь' }}</b><small>{{ new Date(item.created_at).toLocaleString('ru') }}</small><p>{{ item.content }}</p></div></div><form class="comment-form" @submit.prevent="addComment"><input v-model="comment" placeholder="Напишите комментарий" required/><button class="primary">Отправить</button></form></section></div>
